...
describe('should index RDPC analyses to donor index', () => {
	...
	const mockAnalysisFetcher: typeof fetchAnalyses = async ({
		...
		const matchesDonorId = (donor: any) => (donorId ? donor.donorId === donorId : true);
	...
	it('should index sequencing experiment and sequencing alignment analyses', async () => {
		...
		for (const hit of esHits) {
			...
			expect(hit._source.mutectFailed).to.equal(expectedRDPCData[hit._source.donorId].mutectFailed);
	...
	it('should handle incremental rdpc indexing with studyId', async () => {
		...
		await indexRdpcData({
			...
			analysesWithSpecimensFetcher: mockAnalysesWithSpecimensFetcher,
			fetchVC: mockVariantCallingFetcher,
			fetchDonorIds: ({ analysisId, rdpcUrl }) => Promise.resolve([testDonorId]),
		});
		...
		esHits.forEach((hit) => {
			...
			expect([hit._source.donorId, 'alignmentsFailed', hit._source.alignmentsFailed]).to.deep.equal(
				[
					hit._source.donorId,
					'alignmentsFailed',
					hit._source.donorId === testDonorId
			...
			expect([hit._source.donorId, 'sangerVcsRunning', hit._source.sangerVcsRunning]).to.deep.equal(
				[
					hit._source.donorId,
					'sangerVcsRunning',
					hit._source.donorId === testDonorId
			...
			expect([hit._source.donorId, 'openAccessFailed', hit._source.openAccessFailed]).to.deep.equal(
				[
					hit._source.donorId,
					'openAccessFailed',
					hit._source.donorId === testDonorId
