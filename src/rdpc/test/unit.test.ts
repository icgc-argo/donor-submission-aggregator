...
import { donorWithMatchedSamplePairs, mergedDonorDataMap } from './fixtures/DonorSpecimen/testData';
import {
	donorByVCRunState,
...
describe('RDPC sequencing experiment analyses processing', () => {
	it('should remove COMPLETE runs with SUPPRESSED producedAnalyses ', () => {
		const result = removeCompleteRunsWithSuppressedAnalyses(analysesWithEmptyProducedAnalyses);
		expect(JSON.stringify(result)).to.equal(JSON.stringify(removeSuppressedAnalyses_expected));
	});

	it('should not remove RUNNING AND EXECTOR_ERROR runs', () => {
		const result_1 = removeCompleteRunsWithSuppressedAnalyses(analysesWithActiveRunsOnly);
		expect(JSON.stringify(result_1)).to.equal(JSON.stringify(analysesWithActiveRunsOnly));
	...
	it('converts and merges sequencing experiment analyses to a donor document map', async () => {
		const donorCentric_page_1 = toDonorCentric(seqExpAnalysesWithMultipleRuns_page_1);

		expect(JSON.stringify(donorCentric_page_1)).to.equal(
			JSON.stringify(donorCentric_page_1_expected),
		);

		const donorCentric_page_2 = toDonorCentric(seqExpAnalysesWithMultipleRuns_page_2);
		expect(JSON.stringify(donorCentric_page_2)).to.equal(
			JSON.stringify(donorCentric_page_2_expected),
		);

		const allMerged = await getAllRunsByAnalysesByDonors(donorCentric_page_1, donorCentric_page_2);
		expect(JSON.stringify(allMerged)).to.equal(JSON.stringify(mergedPage_expected));
	});

	it('should return the latest run', async () => {
		const latestRun_1 = getLatestRun(runsWithMultipleStates_1);
		expect(JSON.stringify(latestRun_1)).to.equal(JSON.stringify(latestRun_expected_1));
	...
	it('should count the latest alignment run state for each donor', async () => {
		...
		expect(JSON.stringify(donorState)).to.equal(JSON.stringify(donorStateMap_expected));
	});
});
...
describe('RDPC sequencing alignment analyses processing', () => {
	it(
		...
		async () => {
			...
			expect(JSON.stringify(converted)).to.equal(JSON.stringify(donorCentricWithOneTNPair));
		},
	);

	it('should convert sequencing alignment analyses to donor centric map', async () => {
		const donorCentric_page_1 = toDonorCentric(seqAlignAnalysesWithMultiTNPairs_page_1);
		expect(JSON.stringify(donorCentric_page_1)).to.equal(
			JSON.stringify(donorCentricWithMultipleTNPairs_page_1),
		);

		const donorCentric_page_2 = toDonorCentric(seqAlignAnalysesWithMultiTNPairs_page_2);
		expect(JSON.stringify(donorCentric_page_2)).to.equal(
			JSON.stringify(donorCentricWithMultipleTNPairs_page_2),
		);

		const mergedPages = getAllRunsByAnalysesByDonors(donorCentric_page_1, donorCentric_page_2);
		expect(JSON.stringify(mergedPages)).to.equal(JSON.stringify(mergedDonorByAlignRuns));
	});
	...
	it('should count the latest sanger VC run state for each donor', async () => {
		...
		expect(JSON.stringify(donorState)).to.equal(JSON.stringify(donorByVCRunState));
	});
});
...
describe('RDPC sequencing experiment analyses with specimen/sample processing', () => {
	it('should find matched tumour/normal sample pairs', async () => {
		...
		expect(JSON.stringify(matchedPairs)).to.equal(JSON.stringify(matchedSamplePairs_expected));
	});
	...
	it('should find the earliest available sample pair', async () => {
		...
		expect(JSON.stringify(result)).to.equals(JSON.stringify(donorsWithEarliestPair_expected));
	});
});
