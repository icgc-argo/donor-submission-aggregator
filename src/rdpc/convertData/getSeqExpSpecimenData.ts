...
export const getSeqExpSpecimenData = async (
	...
	const rdpcInfo_samplePairsCount = countMatchedSamplePairs(matchedSamplePairsByDonorId_seqExp);
	...
	const rdpcInfo_dnaRawReads = mergeDonorInfo(rdpcInfo_TNcounts, rdpcInfo_rawReadsDate);
	const rdpcInfo_dna = mergeDonorInfo(rdpcInfo_dnaRawReads, rdpcInfo_samplePairsCount);
	const result = mergeDonorInfo(rdpcInfo_dna, rdpcInfo_rnaRawReadsFirstPublishedDate);
	return result;
};
