 ...
 */
export const toDonorCentric = (analyses: Analysis[]): RunsByAnalysesByDonors => {
	const result = analyses.reduce<RunsByAnalysesByDonors>((runAccWithDuplicateDonors, analysis) => {
		...
		Object.entries(donorWithLatestRun).forEach(([donorId, inputAnalysesMap]) => {
...
export const getLatestRun = (runs: Run[]): Run | undefined => {
	return _(runs)
		.sortBy((run) => ({ COMPLETE: 1, RUNNING: 2, EXECUTOR_ERROR: 3 })[run.state])
		.head();
};
	...
	if (donorIds) {
		for (const donorId of donorIds) {
			...
			for await (const page of stream) {
				logger.info(`Streaming ${page.length} of ${analysisType} analyses for ${workflowName}...`);
...
export const countSpecimenType = (map: StringMap<DonorData>): DonorInfoMap => {
	...
	Object.entries(map).forEach(([donorId, donorData]) => {
		...
		for (const normal of normalSpecimen) {
			if (normal.samples[0]?.sampleType.toUpperCase().includes(DNA_SAMPLE_TYPE_KEYWORD)) {
				if (result[donorId]) {
			...
			if (normal.samples[0]?.sampleType.toUpperCase().includes(RNA_SAMPLE_TYPE_KEYWORD)) {
				if (result[donorId]) {
		...
		for (const tumour of tumourSpecimen) {
			if (tumour.samples[0]?.sampleType.toUpperCase().includes(DNA_SAMPLE_TYPE_KEYWORD)) {
				if (result[donorId]) {
			...
			if (tumour.samples[0]?.sampleType.toUpperCase().includes(RNA_SAMPLE_TYPE_KEYWORD)) {
				if (result[donorId]) {
...
export const countMutectRunState = (donorMap: RunsByAnalysesByDonors): DonorInfoMap => {
	const result: DonorInfoMap = {};
...
export const removeCompleteRunsWithSuppressedAnalyses = (analyses: Analysis[]): Analysis[] => {
	const result = analyses.reduce<Analysis[]>((aanalysisAccumulator, analysis) => {
...
export const countRnaAlignmentRunState = (donorMap: RunsByAnalysesByDonors): DonorInfoMap => {
	const result: DonorInfoMap = {};
	Object.entries(donorMap).forEach(([donorId, map]) => {
		Object.entries(map).forEach(([inputAnalysesId, runs]) => {
			runs.forEach((run) => {
...
export const countAlignmentRunState = (donorMap: RunsByAnalysesByDonors): DonorInfoMap => {
	const result: DonorInfoMap = {};
	Object.entries(donorMap).forEach(([donorId, map]) => {
		Object.entries(map).forEach(([inputAnalysesId, runs]) => {
			runs.forEach((run) => {
...
export const countVCRunState = (donorMap: RunsByAnalysesByDonors): DonorInfoMap => {
	const result: DonorInfoMap = {};
	Object.entries(donorMap).forEach(([donorId, map]) => {
		Object.entries(map).forEach(([inputAnalysesId, runs]) => {
			runs.forEach((run) => {
...
export const countOpenAccessRunState = (donorMap: RunsByAnalysesByDonors): DonorInfoMap => {
	const result: DonorInfoMap = {};
	Object.entries(donorMap).forEach(([donorId, map]) => {
		Object.entries(map).forEach(([inputAnalysesId, runs]) => {
			runs.forEach((run) => {
...
export const initializeRdpcInfo = (result: DonorInfoMap, donorId: string): void => {
...
export const mergeDonorInfo = (map: DonorInfoMap, mergeWith: DonorInfoMap): DonorInfoMap => {
	const result = Object.entries(mergeWith).reduce<DonorInfoMap>((acc, [donorId, rdpcInfo]) => {
		acc[donorId] = {
			matchedTNPairsDNA: (acc[donorId]?.matchedTNPairsDNA || 0) + rdpcInfo.matchedTNPairsDNA,

			rnaPublishedNormalAnalysis:
				(acc[donorId]?.rnaPublishedNormalAnalysis || 0) + rdpcInfo.rnaPublishedNormalAnalysis,
			rnaPublishedTumourAnalysis:
				(acc[donorId]?.rnaPublishedTumourAnalysis || 0) + rdpcInfo.rnaPublishedTumourAnalysis,
			rnaRawReadsFirstPublishedDate: acc[donorId]?.rnaRawReadsFirstPublishedDate
				? acc[donorId].rnaRawReadsFirstPublishedDate
				: rdpcInfo.rnaRawReadsFirstPublishedDate,

			rnaAlignmentsCompleted:
				(acc[donorId]?.rnaAlignmentsCompleted || 0) + rdpcInfo.rnaAlignmentsCompleted,
			rnaAlignmentsRunning:
				(acc[donorId]?.rnaAlignmentsRunning || 0) + rdpcInfo.rnaAlignmentsRunning,
			rnaAlignmentFailed: (acc[donorId]?.rnaAlignmentFailed || 0) + rdpcInfo.rnaAlignmentFailed,
			rnaAlignmentFirstPublishedDate: acc[donorId]?.rnaAlignmentFirstPublishedDate
				? acc[donorId].rnaAlignmentFirstPublishedDate
				: rdpcInfo.rnaAlignmentFirstPublishedDate,

			publishedNormalAnalysis:
				(acc[donorId]?.publishedNormalAnalysis || 0) + rdpcInfo.publishedNormalAnalysis,
			publishedTumourAnalysis:
				(acc[donorId]?.publishedTumourAnalysis || 0) + rdpcInfo.publishedTumourAnalysis,
			...
			alignmentsCompleted: (acc[donorId]?.alignmentsCompleted || 0) + rdpcInfo.alignmentsCompleted,
			alignmentsRunning: (acc[donorId]?.alignmentsRunning || 0) + rdpcInfo.alignmentsRunning,
			alignmentsFailed: (acc[donorId]?.alignmentsFailed || 0) + rdpcInfo.alignmentsFailed,
			...
			sangerVcsCompleted: (acc[donorId]?.sangerVcsCompleted || 0) + rdpcInfo.sangerVcsCompleted,
			sangerVcsRunning: (acc[donorId]?.sangerVcsRunning || 0) + rdpcInfo.sangerVcsRunning,
			sangerVcsFailed: (acc[donorId]?.sangerVcsFailed || 0) + rdpcInfo.sangerVcsFailed,
			...
			mutectCompleted: (acc[donorId]?.mutectCompleted || 0) + rdpcInfo.mutectCompleted,
			mutectRunning: (acc[donorId]?.mutectRunning || 0) + rdpcInfo.mutectRunning,
			...
			openAccessCompleted: (acc[donorId]?.openAccessCompleted || 0) + rdpcInfo.openAccessCompleted,
			openAccessRunning: (acc[donorId]?.openAccessRunning || 0) + rdpcInfo.openAccessRunning,
			openAccessFailed: (acc[donorId]?.openAccessFailed || 0) + rdpcInfo.openAccessFailed,
			...
			totalFilesCount: (acc[donorId]?.totalFilesCount || 0) + rdpcInfo.totalFilesCount,
			filesToQcCount: (acc[donorId]?.filesToQcCount || 0) + rdpcInfo.filesToQcCount,
