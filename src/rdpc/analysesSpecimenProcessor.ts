...
export const getAllMergedDonorWithSpecimens = async ({
	...
	if (donorIds) {
		for (const donorId of donorIds) {
			logger.info(`Streaming ${analysisType} analyses with Specimens for donor ${donorId}`);
			...
			for await (const page of stream) {
				...
				const donorDataMapPerPage: StringMap<DonorData> = convertAnalysisToDonorData(page);
...
export const getFirstPublishedDate = (
	...
	Object.entries(samplePairs).forEach(([donorId, pair]) => {
		if (pair.firstPublishedAt !== 0) {
			...
			if (fieldName === FirstPublishedDateFields.RAW_READS_FIRST_PUBLISHED_DATE) {
				donorInfo[donorId].rawReadsFirstPublishedDate = new Date(pair.firstPublishedAt);
			}

			if (fieldName === FirstPublishedDateFields.ALIGNMENT_FIRST_PUBLISHED_DATE) {
				donorInfo[donorId].alignmentFirstPublishedDate = new Date(pair.firstPublishedAt);
			}
		}
	});
	return donorInfo;
...
const convertAnalysisToDonorData = (analyses: Analysis[]): StringMap<DonorData> => {
	const donorDataMapPerPage: StringMap<DonorData> = analyses.reduce<StringMap<DonorData>>(
		(donorDataAccumulator, analysis) => {
			analysis.donors.forEach((donor) => {
				...
				const donorData = getDonorData(donor, analysis.firstPublishedAt, experimentStrategy);
...
const mergeDonorData = (map: StringMap<DonorData>, donorData: DonorData) => {
	if (map[donorData.donorId]) {
		map[donorData.donorId].specimen = [...map[donorData.donorId].specimen, ...donorData.specimen];
...
const getDonorData = (
	...
	const flattenedSample: FlattenedSample = {
		...
		matchedNormalSubmitterSampleId:
			sample.matchedNormalSubmitterSampleId === null ? '' : sample.matchedNormalSubmitterSampleId,
	...
	const samplePair: SamplePair =
		flattenedSample.tumourNormalDesignation === TumourNormalDesignationValue.Normal
			? {
...
const mergeAllPagesDonorData = (merged: StringMap<DonorData>, toMerge: StringMap<DonorData>) => {
	Object.entries(toMerge).forEach(([donorId, donorData]) => {
		if (merged[donorId]) {
			merged[donorId].specimen = [...merged[donorId].specimen, ...donorData.specimen];
			merged[donorId].samplePairs = [...merged[donorId].samplePairs, ...donorData.samplePairs];
		} else {
