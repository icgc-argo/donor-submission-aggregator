...
export default async (programShortName: string, targetIndexName: string, esClient: Client) => {
	const donorStream = Clinical.fetchAllDonorsForProgram(programShortName);
	let donorCount = 0;
	for await (const donor of donorStream) {
		const esHits = await queryDocumentsByDonorIds([donor.donorId], esClient, targetIndexName);
	...
	logger.info(`Indexed Clinical Data for ${donorCount} donors for program ${programShortName}`);
};
