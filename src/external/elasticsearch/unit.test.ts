...
describe('toEsBulkIndexActions', () => {
	it('must transform properly', () => {
		...
		expect(
			toEsBulkIndexActions<(typeof docs)[0]>(TEST_INDEX, (doc) => String(doc.id))(docs),
		).to.deep.equal([
...
describe('initIndexMapping', () => {
	...
	it('must initialize index mappping properly', async () => {
		...
		expect(mappingResponse.body[TEST_INDEX].mappings).to.deep.equal(donorIndexMapping.mappings);
	});
});
