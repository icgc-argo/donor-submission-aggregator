...
export const toEsBulkIndexActions =
	<T = {}>(indexName: string, getDocumentId: (document: T) => string | undefined) =>
	(docs: Array<T>) =>
		flatMap(docs, (doc) => {
...
export const getLatestIndexName = async (esClient: Client, programId: string): Promise<string> => {
	const result = (await esClient.indices.getAlias({
		name: rollcallConfig.aliasName,
	})) as AliasResponse;

	const indexNameList = Object.entries(result.body).map(([indexName, alias]) => {
		return indexName;
	});
