...
function createConsumer(
	...
	const sendDlqMessage = async (messageJSON: string) => {
		if (config.dlq && dlqProducer) {
			...
			logger.warn(`No DLQ configured for ${config.topic} consumer. Not sending message to a DLQ.`);
		}
	};
	...
	async function handleMessage(
		...
		try {
			...
			logger.error(`Failed to handle program queue message, offset: ${message.offset}`, err);
