...
const parseFilePublicReleaseEvent = (message: string): FilePublicReleaseEvent => {
	const obj = JSON.parse(message);
	if (isFilePublicReleaseEvent(obj)) {
		return obj;
	} else {
		logger.warn('Failed to process message. Message must have a file release id and label.');
		return {
			id: '',
			publishedAt: '',
			label: '',
...
const isFilePublicReleaseEvent = (data: unknown): data is FilePublicReleaseEvent => {
