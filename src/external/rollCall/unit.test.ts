...
describe('rollcall integration', () => {
	...
	it('should create new indices and release them', async () => {
		...
		const shards = Number(indexInfo[newIndexName].settings.index.number_of_shards);
		expect(shards).to.equal(3);

		const replicas = Number(indexInfo[newIndexName].settings.index.number_of_replicas);
