...
export const FILES_STREAM_SIZE =
	!Number(process.env.FILE_STREAM_SIZE) || Number(process.env.FILE_STREAM_SIZE) <= 0
		? 100
		: Number(process.env.FILE_STREAM_SIZE);

export const SEQ_ALIGN_REPO_URL =
	process.env.SEQ_ALIGN_REPO_URL || 'https://github.com/icgc-argo/dna-seq-processing-wfs.git';

export const SANGER_VC_REPO_URL =
	process.env.SANGER_VC_REPO_URL || 'https://github.com/icgc-argo/sanger-variant-calling.git';

export const MUTECT_REPO_URL =
	process.env.MUTECT_REPO_URL || 'https://github.com/icgc-argo/gatk-mutect2-variant-calling.git';
...
const KAFKA_CONSUMER_GROUP = process.env.KAFKA_CONSUMER_GROUP || 'donor-submission-aggregator';
...
const programQueueConsumer: KafkaConsumerConfiguration = {
	group: process.env.KAFKA_PROGRAM_QUEUE_CONSUMER_GROUP || 'program-queue-donor-processor',
	topic: KAFKA_PROGRAM_QUEUE_TOPIC,
	dlq: DLQ_TOPIC_NAME,

	partitionsConsumedConcurrently: Number(process.env.PARTITIONS_CONSUMED_CONCURRENTLY || 5),
	...
	sessionTimeout: Number(process.env.KAFKA_PROGRAM_QUEUE_CONSUMER_SESSION_TIMEOUT) || 120 * 1000,
	...
	rebalanceTimeout:
		Number(process.env.KAFKA_PROGRAM_QUEUE_CONSUMER_REBALANCE_TIMEOUT) || 240 * 1000,
};
...
if (USE_VAULT && Object.entries(REQUIRED_VAULT_CONFIGS).some(missingValue)) {
	const error = new Error(
		`USE_VAULT is true but missing configs: ${Object.entries(REQUIRED_VAULT_CONFIGS)
			.filter(missingValue)
			.map(([key]) => key)}`,
	);
	throw error;
...
if (USE_VAULT && VAULT_AUTH_METHOD === 'kubernetes') {
	if (Object.entries(REQUIRED_K8_CONFIGS).some(missingValue)) {
		const error = new Error(
			`VAULT_AUTH_METHOD is "kubernetes" but missing configs: ${Object.entries(REQUIRED_K8_CONFIGS)
				.filter(missingValue)
				.map(([key]) => key)}`,
		);
		throw error;
...
export const FEATURE_RDPC_INDEXING_ENABLED = process.env.FEATURE_RDPC_INDEXING_ENABLED === 'true';

// Index data from File Service
export const FEATURE_INDEX_FILE_ENABLED = process.env.FEATURE_INDEX_FILE_ENABLED === 'true';
...
export const FEATURE_INDEX_ENDPOINT_ENABLED =
  process.env.FEATURE_INDEX_ENDPOINT_ENABLED === "true";
