...
import { DonorMolecularDataReleaseStatus, File, FileReleaseState } from './types';
	...
	Object.entries(files).forEach(([donorId, files]) => {
		...
		switch (releaseStates.length) {
			case 0:
				...
				result[donorId].releaseStatus = DonorMolecularDataReleaseStatus.NO_RELEASE;

			case 1:
				// FULLY_RELEASED - all files for donor have release_state of PUBLIC
				if (releaseStates[0] === FileReleaseState.PUBLIC) {
					result[donorId].releaseStatus = DonorMolecularDataReleaseStatus.FULLY_RELEASED;
				}
			...
			case 2:
				...
				if (releaseStates.indexOf(FileReleaseState.PUBLIC) != -1) {
					result[donorId].releaseStatus = DonorMolecularDataReleaseStatus.PARTIALLY_RELEASED;
				} else {
					result[donorId].releaseStatus = DonorMolecularDataReleaseStatus.NO_RELEASE;
				}
				break;
	...
	if (donorIds) {
		for (const donorId of donorIds) {
			...
			for await (const page of stream) {
				logger.info(`Streaming ${page.length} of files for donorId ${donorId}...`);
