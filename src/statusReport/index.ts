...
const createStatusReporter = (app: ReturnType<typeof express>) => (endpoint: string) => {
	let state: {
		isReady: boolean;
	...
	return {
		...
		endProcessingProgram: (programId: string) => {
			logger.profile(programId);
			setState({
				processingProgram: state.processingProgram.filter((id) => programId !== id),
			});
		},
	};
};
export default createStatusReporter;
export type StatusReporter = ReturnType<ReturnType<typeof createStatusReporter>>;
