...
describe('kafka integration', () => {
	...
	before(async () => {
		try {
			...
			rollcallContainer = await new GenericContainer('overture/rollcall', '2.5.0')
				.withNetworkMode(NETOWRK_MODE)
	...
	const createIndexAndAlias = async (programId: string) => {
		...
		console.log(`expecting index ${firstIndexName} to have alias ${ROLLCALL_ALIAS_NAME}...`);
	...
	describe('programQueueProcessor', () => {
		it('must index all clinical and RDPC data into Elasticsearch', async () => {
			...
			const query_test_ca = esb.requestBodySearch().query(esb.termQuery('programId', TEST_CA));
			...
			console.log(`expecting test_ca_documents_clinical to be ${testDonorIds.length}`);
			...
			console.log(`expecting test_ca_documents_rdpc to be ${testDonorIds.length}`);
			...
			for (const test_ca_hit of hits) {
				...
				console.log(`expecting TEST-CA donor id = ${donorId} to have 1 es hit...`);
				...
				expect(test_ca_hit.hits[0]._source.sangerVcsFirstPublishedDate).to.equal(
				...
				expect(test_ca_hit.hits[0]._source.openAccessFirstPublishedDate).to.equal(
					expectedRDPCData['DO' + donorId].openAccessFirstPublishedDate,
				);
			}

			// check if the number of TEST-US documents is expected:
			const query_test_us = esb.requestBodySearch().query(esb.termQuery('programId', TEST_US));
			...
			expect(totalEsDocuments).to.equal(testDonorIds.length + DB_COLLECTION_SIZE);
		});
		...
		it('must create new index with correct settings and index data', async () => {
			...
			expect(currentNumOfReplicas).to.equal(donorIndexMapping.settings['index.number_of_replicas']);
			expect(currentNumOfShards).to.equal(donorIndexMapping.settings['index.number_of_shards']);
			...
			const indexSettings_1 = response_2.body[existingIndexName_1].settings.index;
			...
			expect(currentNumOfShards_1).to.equal(donorIndexMapping.settings['index.number_of_shards']);
		...
		it(
			...
			async () => {
				...
				// bulk insert data:
				const body = clinicalDataset.flatMap((doc) => [{ index: { _index: firstIndexName } }, doc]);
		...
		it('handles incremental analysis updates properly', async () => {
			...
			esHits.forEach((hit) => {
				...
				expect([hit._source.donorId, 'sangerVcsFailed', hit._source.sangerVcsFailed]).to.deep.equal(
					[
						hit._source.donorId,
						'sangerVcsFailed',
						hit._source.donorId === testDonorId
				...
				expect([hit._source.donorId, 'mutectCompleted', hit._source.mutectCompleted]).to.deep.equal(
					[
						hit._source.donorId,
						'mutectCompleted',
						hit._source.donorId === testDonorId
				...
				expect([hit._source.donorId, 'mutectRunning', hit._source.mutectRunning]).to.deep.equal([
					hit._source.donorId,
					'mutectRunning',
					hit._source.donorId === testDonorId
				...
				expect([hit._source.donorId, 'mutectFailed', hit._source.mutectFailed]).to.deep.equal([
					hit._source.donorId,
					'mutectFailed',
					hit._source.donorId === testDonorId
